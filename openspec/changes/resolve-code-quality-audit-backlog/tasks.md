...
- [ ] 1.1 关闭 `G-T0001`：WebDAV 默认启用严格 TLS，提供可控开关（用户显式；Release 允许）
- [ ] 1.2 关闭 `G-T0002`：收敛/下线 `UnsafeOkHttpClient`，提供更安全替代方案与文档
...
- [ ] 1.11 关闭 `G-T0011`：遥测聚合/事件构建逻辑迁出 Base（`data_component` 仅保留 payload）
- [ ] 1.12 关闭 `G-T0012`：建立统一脱敏工具并迁移关键链路调用点（网络/播放器/缓存/下载）
- [ ] 1.13 关闭 `G-T0013`：错误上报脱敏 + 登录/用户资料上报改造（避免泄露个人信息）
- [ ] 1.14 关闭 `G-T0014`：开启 Room schema 导出 + 迁移校验门禁
- [ ] 1.15 关闭 `G-T0015`：抽取 TV Tab 焦点协调与输入策略组件，统一 DPAD 行为
...
- [ ] 2.1 关闭 `G-T0027`：Cookie/Token 加密存储（密钥管理 + 迁移）
...
- [ ] 2.28 关闭 `G-T0054`：移除“发送弹幕”整条链路（删除相关 UI/代码并移除依赖）
...
- [ ] 2.32 关闭 `G-T0058`：统一 Service 包结构（service/services）
