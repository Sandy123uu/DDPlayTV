...
**Tests**: Testing tasks are REQUIRED for any behavior change (unit/integration/contract as appropriate). If a story is documentation-only, state why test tasks are not needed.
...
- [ ] T004 Add Constitution Check outputs to plan (module impact, TV/DPAD impact, validation commands)
...
- [ ] T011 Add dependency-governance validation task (`./gradlew verifyModuleDependencies`) when module edges change
- [ ] T012 Define observability + sensitive-data redaction checklist for this feature
...
- [ ] TXXX Execute required Gradle gates (e.g., `testDebugUnitTest`, `assembleDebug`) and capture BUILD SUCCESSFUL/FAILED tail result
- [ ] TXXX Run TV DPAD regression checklist for UI-impacting stories
...
- Record build/test command tails with explicit BUILD SUCCESSFUL/FAILED conclusion
