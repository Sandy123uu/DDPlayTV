...
- 模块职责（不做什么）
  ...
  - 不应在没有明确产品策略的情况下默认使用“不安全 TLS”（信任所有证书/关闭主机名校验）的 OkHttpClient，除非受 debug/用户显式开关控制。
- 关键入口/关键符号（示例）
