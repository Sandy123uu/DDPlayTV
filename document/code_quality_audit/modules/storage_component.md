...
|---|---|---|---|---|---|---|---|---|---|
| STORAGE-T001 | STORAGE-F001 | 移除投屏 Sender 整条链路：删除旧实现与全端 stub，清理入口/路由/服务暴露 | 删除 `ScreencastProvideService.kt` 与 `ScreencastProvideServiceImpl.kt`（以及相关入口/路由/依赖暴露点），并清理注释旧实现；如未来需要 Sender，另行立项恢复 | 1) 全仓无 Sender 入口与服务暴露（不再存在“全端 stub + 对外仍暴露接口”形态）；2) 不保留大段注释旧实现；3) 投屏接收端（Receiver）链路不受影响；4) 全仓编译通过 | High | Medium | P1 | 待分配（Storage/TV） | Draft |
