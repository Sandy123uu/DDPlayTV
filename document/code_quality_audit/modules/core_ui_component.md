...
| CORE_UI-T003 | CORE_UI-F003 | 明确 Fragment 宿主契约：引入 `LoadingHost`（或等价接口）并替换强制 cast，避免隐藏崩溃点 | `BaseAppFragment` + 所有继承链（`BaseFragment/BasePreferenceFragmentCompat` 等）及其宿主 Activity | 1) 不再出现 `context as BaseAppCompatActivity<*>` 的强制转换；2) 宿主不满足契约时能明确报错（Fail Fast）；3) 全仓编译通过，页面 loading 行为不变 | Medium | Small | P1 | 待分配（UI） | Draft |
