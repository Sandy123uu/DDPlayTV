...
| BILIBILI-F003 | ArchitectureRisk | `BilibiliRepository` 单类过大且多职责（登录/风控/播放/历史/直播弹幕等），演进与测试成本高 | 已拆分：`bilibili_component/src/main/java/com/xyoye/common_component/bilibili/repository/BilibiliRepositoryCore.kt` + 子域 facade（`BilibiliAuthRepository/BilibiliRiskRepository/BilibiliPlaybackRepository/BilibiliHistoryRepository/BilibiliLiveRepository/BilibiliDanmakuRepository`） | N/A | Unify | `:bilibili_component`（按子域拆分：Auth/Risk/Playback/History/Live 等） | Medium | Large | P2 | 保留 `BilibiliRepository` facade 兼容调用侧，支持渐进迁移 |
