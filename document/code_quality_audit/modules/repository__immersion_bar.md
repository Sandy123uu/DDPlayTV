...
- 关键入口/关键路径（示例）
  ...
  - 依赖声明（出口）：`core_ui_component/build.gradle.kts` + `implementation(project(":repository:immersion_bar"))`（通过 `StatusBarStyle` 对外收敛）
...
- 依赖边界
  - 对外（被依赖）：当前由 `:core_ui_component` 内部 `implementation(...)` 引入，第三方类型不再向 feature 模块透传；对外统一通过 `StatusBarStyle` 等入口暴露能力。
  - 对内（依赖）：无（仅提供 AAR 工件，不应再依赖其它工程模块）。
  - 边界疑点：
    - 历史风险：当 `:core_ui_component` 以 `api(...)` 方式导出时，第三方类型会扩散到多个模块的编译期 API 面，升级/替换成本线性增长。该风险已由 T002 治理。
...
| REPO_IMMERSION_BAR-F002 | ArchitectureRisk | 第三方类型通过 `core_ui_component` 的 `api(...)` 扩散到多模块，耦合面大且升级成本高 | 历史出口：`core_ui_component/build.gradle.kts` + `api(project(":repository:immersion_bar"))`；现状：`core_ui_component/build.gradle.kts` 已改为 `implementation(project(":repository:immersion_bar"))`，并由 `StatusBarStyle` 统一封装调用 | Intentional | Unify | `:core_ui_component`（提供抽象/门面，逐步收敛到统一入口） | Medium | Medium | P2 | 若直接升级/替换库（或 API 变更），需要同时迁移多模块多处调用，回归成本高 |
...
| REPO_IMMERSION_BAR-T002 | REPO_IMMERSION_BAR-F002 | 收敛第三方类型扩散：以 `core_ui_component` 提供统一状态栏/沉浸式配置入口 | 在 `:core_ui_component` 增加统一门面（例如 `StatusBarStyleApplier` / `ImmersionBarFacade`），并将 feature 侧直接 `ImmersionBar.with(...)` 的调用逐步迁移；迁移完成后评估是否可将 `api(project(":repository:immersion_bar"))` 降级为 `implementation(...)` | 1) 新增入口可覆盖现有常见用法（透明/fitSystemWindows/字体颜色等）；2) 至少迁移 2 个调用点验证可行（例如 `BaseActivity` 与 `BaseSplashActivity`）；3) 不影响 TV/移动端交互与样式；4) 迁移过程保持渐进（允许双栈共存一段时间） | Medium | Medium | P2 | AI（Codex） | Done |

### T002 关闭说明（2026-02-06）

- `core_ui_component` 已提供统一入口 `StatusBarStyle`（默认/透明/标题栏透明/全屏等）。
- ast-grep 检索 Kotlin `import com.gyf.immersionbar.$NAME` 仅命中 `core_ui_component` 内部封装文件。
- ast-grep 检索 `ImmersionBar.with($$$ARGS)` 仅命中 `core_ui_component` 的 `StatusBarStyle.kt`，feature 模块改为调用统一入口。
- `core_ui_component/build.gradle.kts` 已将 `:repository:immersion_bar` 调整为 `implementation(...)`，第三方类型不再向上层模块透传。
