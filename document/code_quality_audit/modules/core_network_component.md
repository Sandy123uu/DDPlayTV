...
- 模块职责（不做什么）
  ...
  - 不应在默认路径中降低 TLS 安全（证书/主机名校验），除非有明确的 debug/用户显式开关策略。
- 关键入口/关键路径（示例）
