...
| REPO_PANEL_SWITCH-T002 | REPO_PANEL_SWITCH-F002 | 明确“发送弹幕”能力是否仍在产品范围：移除无效链路或恢复并补齐策略 | 若确认全端禁用：删除 `SendDanmuDialog` 与 `layout_send_danmu.xml`，移除 `player_component/build.gradle.kts` 对 `:repository:panel_switch` 的依赖；若确认仍需保留：恢复入口并补齐 TV/移动端交互策略与回归用例 | 1) 依赖与调用点一致（无“依赖存在但入口不可达”）；2) 若移除，`rg "com\\.effective\\.android\\.panel" -n` 无命中；3) 若保留，入口可达且回归用例覆盖键盘/面板切换与返回逻辑 | Medium | Medium | P2 | 待分配（Player） | Draft |
