...
- 模块职责（做什么）
  ...
  - 提供面板/键盘切换能力（第三方包：`com[.]effective[.]android[.]panel.*`），典型用于“输入框 + 扩展面板（表情/样式面板等）”的切换与高度联动。
- 模块职责（不做什么）
...
- 关键入口/关键路径（示例）
  ...
  - 历史依赖声明（已移除）：`:player_component` 曾依赖该 wrapper（见 `REPO_PANEL_SWITCH-T002`）
  - 历史使用点（已移除）：发送弹幕对话框 + 布局（见 `REPO_PANEL_SWITCH-T002`）
  - License 线索：`user_component/src/main/assets/license/PanelSwitchHelper.txt`（含上游地址）；概览清单：`document/Third_Party_Libraries.md`
- 依赖边界
  - 对外（被依赖）：当前工程内未发现任何模块直接依赖该 wrapper（已按 `REPO_PANEL_SWITCH-T002` 清理发送弹幕链路）。
  - 对内（依赖）：无（仅提供 AAR 工件，不应再依赖其它工程模块）。
  - 边界疑点：
    - 发送弹幕死链路已按 `REPO_PANEL_SWITCH-T002` 删除；若未来重新引入“输入框 + 面板切换”能力，需重新评估依赖合理性与生命周期释放策略。
...
| REPO_PANEL_SWITCH-F002 | Redundancy | 依赖处于“编译期存在、运行期不可达”：发送弹幕入口被禁用，相关 UI/代码疑似废弃 | 该 Finding 已通过 `REPO_PANEL_SWITCH-T002` 清理：移除发送弹幕整条链路与 wrapper 依赖，避免“编译期存在、运行期不可达” | N/A | Deprecate | `:player_component`（直接移除整条链路：删除相关 UI/代码并移除依赖） | Medium | Medium | P2 | 产品决策已确认：直接移除整条链路；无需为 TV/移动端补齐交互策略 |
| REPO_PANEL_SWITCH-F003 | ArchitectureRisk | 调用侧未持有 `PanelSwitchHelper` 实例，生命周期/资源释放不可控（潜在泄漏/窗口监听残留） | 该风险随“发送弹幕”链路移除而消失；若未来重新引入 PanelSwitchHelper，需要确保 helper 可释放并与宿主生命周期绑定 | N/A | Unify | `:player_component`（若继续保留该能力，封装为可释放的成员并与 Dialog 生命周期绑定） | Medium | Small | P2 | 需要查阅上游库的释放方式（如 `reset()/onDestroy()` 等）；否则可能出现 window listener 残留或内存泄漏 |
