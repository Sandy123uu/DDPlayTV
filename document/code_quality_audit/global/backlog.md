...
| G-T0025 | Done | AI（Codex） | 统一手动迁移异常可观测性：替换 `printStackTrace()` 为结构化日志/异常上报，并补充必要上下文 | G-F0014 | :core_database_component | CORE_DATABASE-T003 |
| G-T0026 | Done | AI（Codex） | 统一错误上报上下文的构建与脱敏策略，减少重复样板并降低隐私风险 | G-F0008 | :core_log_component,:local_component | LOCAL-T003 |
...
| G-T0034 | Done | AI（Codex） | 将 B 站弹幕解析/下载编排从 ViewModel 下沉到 repository/usecase，降低 UI 耦合 | G-F0037 | :local_component,:core_storage_component | LOCAL-T004 |
...
| G-T0039 | Done | AI（Codex） | 引入“数据库访问契约/Provider”，逐步替换跨模块对 `DatabaseManager.instance` 的直接引用 | G-F0041 | :core_database_component | CORE_DATABASE-T001 |
| G-T0040 | Done | AI（Codex） | 打通“写入层磁盘错误→全局状态”链路，确保 `LogSystem`/持久化/Writer 三者一致 | G-F0045 | :core_log_component | CORE_LOG-T002 |
| G-T0041 | Done | AI（Codex） | 抽取 PlayHistory/MediaLibrary 的 repository/usecase，收敛 DAO 访问与写入口径 | G-F0034 | :local_component,:core_database_component | LOCAL-T002 |
| G-T0042 | Draft | AI（Codex） | 抽取 `LogLevel` 优先级/比较逻辑为单一实现，移除重复 `levelPriority` | G-F0051 | :core_log_component | CORE_LOG-T001 |
| G-T0043 | Done | AI（Codex） | 抽取可复用的 PreferenceDataStore/映射抽象，减少 key→config 样板与 drift | G-F0063 | :user_component | USER-T001 |
| G-T0044 | Done | AI（Codex） | 抽取壳层 Fragment 装载/切换逻辑，减少重复实现与差异漂移 | G-F0050 | :app | APP-T003 |
| G-T0045 | Done | AI（Codex） | 抽取投屏协议与 server 公共能力到 core 层，减少 NanoHTTPD/UDP 重复与策略漂移 | G-F0053 | :storage_component,:core_storage_component | STORAGE-T003 |
...
| G-T0048 | Done | AI（Codex） | 拆分 `BilibiliRepository`：按子域抽取组件并引入单测/契约化接口，提高可维护性 | G-F0038 | :bilibili_component | BILIBILI-T003 |
...
| G-T0051 | Done | AI（Codex） | 收敛本地代理/HTTP server 能力，减少多实现与策略漂移 | G-F0054 | :player_component,:core_storage_component | PLAYER-T003,CORE_STORAGE-T005 |
| G-T0052 | Done | AI（Codex） | 收敛第三方协议库依赖泄漏：减少/消除 `api(...)`，上层模块不再直接 import 协议库类型 | G-F0032 | :core_storage_component | CORE_STORAGE-T001 |
...
| G-T0055 | Done | AI（Codex） | 明确登录态单一事实源：用 UserSessionManager 统一 token/登录态更新与观察 | G-F0044 | :user_component | USER-T004 |
...
| G-T0057 | Done | AI（Codex） | 统一 MMKV 初始化策略：明确“Startup 初始化”与“BaseApplication 初始化”的职责边界，避免重复与时序不一致 | G-F0057 | :core_system_component | CORE_SYSTEM-T004 |
...
| G-T0059 | Done | AI（Codex） | 统一 Telemetry repository 的模块归属与包命名，避免“core_network 包名却在 feature 模块” | G-F0012 | :player_component | PLAYER-T004 |
...
| G-T0062 | Done | AI（Codex） | 统一本地代理能力：抽取通用 Proxy 服务（headers/range/tls 策略一致），减少 `HttpPlayServer`/`VlcProxyServer` 逻辑重复 | G-F0055 | :core_storage_component,:player_component | CORE_STORAGE-T005,PLAYER-T003 |
