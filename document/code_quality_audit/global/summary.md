...
|---|---|---|---|---|---|---|---|---|
| G-T0001 | WebDAV 默认启用严格 TLS：替换 `UnsafeOkHttpClient`，并为“特殊场景”提供可控开关（用户显式；Release 允许） | G-F0001 | `core_storage_component/.../WebDavStorage.kt`；`core_network_component/...`（OkHttpClientFactory/安全策略） | 1) release 默认不再使用“信任所有证书”的客户端；2) 若需要放宽校验，必须是用户显式开关（Release 允许）；3) WebDAV 连接/列表/播放用例可回归 | P1 | 待分配（Infra/Network/Storage） | Draft | - |
