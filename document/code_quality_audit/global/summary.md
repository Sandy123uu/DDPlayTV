...
| G-T0034 | 将 B 站弹幕解析/下载编排从 ViewModel 下沉到 repository/usecase，降低 UI 耦合 | G-F0037 | 抽取 `BilibiliDanmuUseCase`（解析 URL→CID 列表、下载 XML、保存）到 `:bilibili_component` 或 `:core_storage_component`；ViewModel 仅负责 UI 状态与消息流 | 1) ViewModel 不包含 Jsoup.connect/正则抽取 JSON 等解析细节；2) BV/AV/番剧 URL 三条链路功能回归；3) 异常上报上下文符合脱敏策略；4) 全仓编译通过 | P2 | AI（Codex） | Done | 2026-02-06：新增 `BilibiliDanmuUseCase` 下沉 URL 解析/CID 提取/弹幕下载保存流程；`BilibiliDanmuViewModel` 移除 Jsoup/JSON/正则细节并仅保留 UI 消息编排；异常上报使用脱敏 URL。 |
...
| G-T0036 | 将字幕匹配/搜索/Hash 能力从 `:local_component` 收敛到基础层，避免 feature 锁死复用 | G-F0056 | 将 `local_component/src/main/java/com/xyoye/common_component/utils/subtitle/*` 迁移到 `:core_storage_component`（或明确的 core 模块）并整理包结构；`BindSubtitleSourceFragmentViewModel` 只依赖 core 层 API | 1) `:local_component` 不再包含 `com.xyoye.common_component.utils.subtitle` 的实现文件；2) 本地视频字幕“匹配/搜索/下载/解压/绑定”功能回归正常；3) 不引入 feature ↔ feature 依赖；4) 全仓编译通过 | P2 | AI（Codex） | Done | 2026-02-06：将 `SubtitleHashUtils/SubtitleSearchHelper/SubtitleMatchHelper` 下沉至 `:core_storage_component`，并补齐该模块 Paging/Lifecycle 依赖；`local_component` 不再持有字幕工具实现，`:app:assembleDebug` 通过。 |
| G-T0037 | 将扫描扩展目录/过滤配置与刷新逻辑下沉到 repository/usecase，统一 DAO 访问口径 | G-F0042 | 在 `:core_database_component` 提供 `ScanSettingsRepository`（ExtendFolder/FolderFilter CRUD）；在 `:core_storage_component` 提供 `VideoScanRefreshUseCase`（触发刷新）；ViewModel 仅编排 UI 状态 | 1) ViewModel 不直接访问 `DatabaseManager.instance.get*Dao()`；2) 扩展目录增删、过滤开关与刷新行为一致；3) 线程调度一致；4) 全仓编译通过 | P2 | AI（Codex） | Done | 2026-02-06：新增 `ScanSettingsRepository` 与 `VideoScanRefreshUseCase`，并迁移 `ScanExtendFragmentViewModel`/`ScanFilterFragmentViewModel` 移除 DAO 直连。 |
| G-T0038 | 将搜索历史/播放历史等持久化细节从 ViewModel 迁移到 repository/usecase 层 | G-F0033 | 建立 `AnimeSearchHistoryRepository` / `MagnetSearchHistoryRepository` / `EpisodeHistoryRepository`（落点建议 `:core_database_component`），ViewModel 仅调用抽象 API | 1) ViewModel 不再直接调用 `DatabaseManager.instance.get*Dao()`；2) repository 的返回类型与现有 LiveData/Flow 对齐；3) 线程调度统一（IO/Main）；4) 功能回归：搜索历史新增/删除/查询、剧集历史补全正常 | P2 | AI（Codex） | Done | 2026-02-06：新增 `AnimeSearchHistoryRepository`、`MagnetSearchHistoryRepository`、`EpisodeHistoryRepository`，并迁移 `SearchAnime/SearchMagnet/AnimeEpisode` 三个 ViewModel 的 DAO 直连调用。 |
| G-T0039 | 引入“数据库访问契约/Provider”，逐步替换跨模块对 `DatabaseManager.instance` 的直接引用 | G-F0041 | 在 `:core_database_component` 新增 `DatabaseProvider` + `DatabaseAccessProvider`（支持测试替换），并迁移 `local_component/storage_component/bilibili_component/player_component/core_storage_component/...` 调用点 | 1) 通过 Provider 获取 DAO（支持测试替换/注入）；2) `./gradlew verifyModuleDependencies` 通过；3) 全仓编译通过 | P2 | AI（Codex） | Done | 2026-02-06：全仓 `DatabaseManager.instance` 调用迁移到 `DatabaseProvider.instance`；`verifyModuleDependencies`、`:app:assembleDebug`、`:core_database_component:testDebugUnitTest` 通过。 |
...
| G-T0041 | 抽取 PlayHistory/MediaLibrary 的 repository/usecase，收敛 DAO 访问与写入口径 | G-F0034 | 在 `:core_database_component` 增加 `PlayHistoryRepository`（历史查询/删除/清空/更新绑定字段）与 `MediaLibraryRepository`；`MediaViewModel`/`PlayHistoryViewModel`/Bind*ViewModel 使用仓库接口 | 1) 目标 ViewModel 不再直接调用 `DatabaseManager.instance.get*Dao()`；2) 行为一致：历史列表/打开历史播放/解绑字幕弹幕；3) 线程调度一致（IO/Main）；4) 全仓编译通过 | P2 | AI（Codex） | Done | 2026-02-06：新增 `PlayHistoryRepository` 与 `MediaLibraryRepository`，并迁移 `MediaViewModel`、`PlayHistoryViewModel`、`BindDanmuSourceFragmentViewModel`、`BindSubtitleSourceFragmentViewModel`、`BindExtraSourceViewModel` 移除 DAO 直连；`:core_database_component:compileDebugKotlin`、`:local_component:compileDebugKotlin`、`:app:assembleDebug` 通过。 |
...
| G-T0043 | 抽取可复用的 PreferenceDataStore/映射抽象，减少 key→config 样板与 drift | G-F0063 | 在 `:core_ui_component` 提供通用 `MappingPreferenceDataStore`（集中 try/catch + 上报）与 key 常量组织方式；迁移 `App/Player/Subtitle/Danmu/Developer` 设置页逐步接入 | 1) 目标 Fragment 内不再手写大量 `when(key)`；2) 旧 key 保持兼容（不丢配置）；3) 关键设置读写行为一致；4) 全仓编译通过 | P2 | AI（Codex） | Done | 2026-02-06：新增 `MappingPreferenceDataStore` 并迁移 App/Player/Subtitle/Danmu/Developer 设置页 DataStore；新增 `MappingPreferenceDataStoreTest` 覆盖映射读写与异常兜底。 |
| G-T0044 | 抽取壳层 Fragment 装载/切换逻辑，减少重复实现与差异漂移 | G-F0050 | 抽取 `FragmentSwitcher`/delegate（建议落在 `:core_ui_component` 或 `:app` 内部包），统一 `findFragmentByTag/add/show/hide` 与状态处理；TV 专有的导航数据结构保留在 `TvMainActivity` | 1) `MainActivity` 与 `TvMainActivity` 使用同一套切换实现；2) 旋转/进程重建后不出现错乱（当前已 `findAndRemoveFragment` 规避）；3) TV DPAD 焦点可达、可见、可返回；4) 不改变现有路由路径与功能入口 | P2 | AI（Codex） | Done | 新增 `ShellFragmentSwitcher` 并接入 Main/TvMain，共享切换实现（2026-02-06）。 |
| G-T0045 | 抽取投屏协议与 server 公共能力到 core 层，减少 NanoHTTPD/UDP 重复与策略漂移 | G-F0053 | 将投屏协议（常量/鉴权/header/range/错误响应）统一到 `:core_storage_component`；`storage_component` 的 UDP/HTTP server 仅保留编排或改为调用 core 的实现 | 1) core 层定义稳定 API；2) feature 不再重复实现同一协议细节；3) 投屏接收链路可回归（发现→鉴权→播放→进度回写）；4) 全仓编译通过 | P2 | AI（Codex） | Done | 2026-02-06：`HttpServer/ServerController/UdpServer` 与 `ScreencastReceiveHandler` 下沉到 `core_storage_component`，`storage_component` 仅保留 `ScreencastReceiveService` 编排；`./gradlew :core_storage_component:compileDebugKotlin :storage_component:compileDebugKotlin` 通过。 |
...
| G-T0048 | 拆分 `BilibiliRepository`：按子域抽取组件并引入单测/契约化接口，提高可维护性 | G-F0038 | `bilibili_component`（新增 `AuthRepository/RiskRepository/PlaybackRepository/HistoryRepository/LiveRepository/DanmakuRepository` 等）；对外保留 facade 以降低调用方改动 | 1) 对外 API 基本不变（或提供 deprecate 迁移期）；2) 关键签名/存储/URL 脱敏具备单测覆盖；3) 全仓编译通过 | P2 | AI（Codex） | Done | 2026-02-06：新增 facade + 子域仓库拆分，`BilibiliRepositoryCore` 下沉实现；新增签名/URL 脱敏/契约单测并通过 `:bilibili_component:testDebugUnitTest` 与 `:app:assembleDebug`。 |
| G-T0049 | 收敛 MediaLibrary/PlayHistory 的写入口径：提供 repository/usecase 并替换 feature 直连 DAO | G-F0031 | `:core_database_component` 新增 `MediaLibraryRepository/PlayHistoryRepository`；迁移 `StoragePlusViewModel/StorageFileFragmentViewModel/ServerController` 等调用点 | 1) 目标类不再直接调用 `DatabaseManager.instance.get*Dao()`；2) 行为一致（存储增删改/续播/绑定字段更新）；3) 线程与事务边界一致；4) 全仓编译通过 | P2 | AI（Codex） | Done | 2026-02-06：`storage_component` 内 `StoragePlusViewModel`、`StorageFileFragmentViewModel`、`Cloud115/Open115/BaiduPan` 编辑对话框全部迁移到 `MediaLibraryRepository/PlayHistoryRepository`，移除 feature 侧 `DatabaseProvider.instance.get*Dao()` 直连。 |
| G-T0050 | 收敛 contract 层的运行时实现：将可变状态/副作用迁移到 runtime 层，仅保留契约 | G-F0030 | `core_contract_component/src/main/java/com/xyoye/common_component/media3/Media3SessionStore.kt`；`core_contract_component/src/main/java/com/xyoye/common_component/config/PlayerActions.kt`；`core_contract_component/src/main/java/com/xyoye/common_component/bridge/*` + 调用方 | 1) contract 层只保留接口/数据类型；2) 迁移后调用方仍可访问同等能力；3) 不引入 feature↔feature 依赖且 `./gradlew verifyModuleDependencies` 通过 | P2 | AI（Codex） | Done | 2026-02-06：`Media3SessionStore/PlayerActions/PlayTaskBridge/ServiceLifecycleBridge` 迁移到 `:core_system_component`，并通过 `verifyModuleDependencies` + `:app:assembleDebug`。 |
| G-T0051 | 收敛本地代理/HTTP server 能力，减少多实现与策略漂移 | G-F0054 | 抽取 “proxy + headers + range + 鉴权 + 错误口径” 到 `:core_storage_component`（或新 core 模块）；player/storage/screencast 复用统一实现 | 1) 同类 server 代码显著减少；2) 多引擎（VLC/mpv/Media3）可播放且 Range/headers 行为一致；3) 默认 TLS 与日志策略统一；4) 全仓编译通过 | P2 | AI（Codex） | Done | 2026-02-06：新增 `LocalProxy` 统一入口（`UpstreamSource/UpstreamTlsPolicy/isServingUrl/setSeekEnabledIfServing`），`WebDav/Alist/BaiduPan/Open115/Cloud115` 及 `Mpv/Vlc/Media3` 播放器全部改用该入口，不再直连 `HttpPlayServer`；`:core_storage_component:compileDebugKotlin`、`:player_component:compileDebugKotlin`、`:app:assembleDebug` 通过。 |
| G-T0052 | 收敛第三方协议库依赖泄漏：减少/消除 `api(...)`，上层模块不再直接 import 协议库类型 | G-F0032 | `core_storage_component/build.gradle.kts`（调整 `api`→`implementation` 或仅保留“确实出现在公共 API 的类型”）；迁移 `storage_component/...` 等直接引用协议库的代码 | 1) 业务模块不再直接依赖 `SMBClient/FTPClient/NanoHTTPD/...` 等第三方类型；2) 依赖声明清晰（需要时显式依赖）；3) 全仓编译通过且关键存储配置/测试连接可用 | P2 | AI（Codex） | Done | 2026-02-06：`core_storage_component` 对 SMB/DCERPC/FTP/WebDAV 库改为 `implementation`；仅 `nano_http` 因公共超类型暂保留 `api`；`storage_component` 不再直接 import `SMBClient`，改用 `StorageProtocolDefaults`。 |
| G-T0053 | 收敛第三方类型扩散：以 `core_ui_component` 提供统一状态栏/沉浸式配置入口 | G-F0047 | 在 `:core_ui_component` 增加统一门面（例如 `StatusBarStyleApplier` / `ImmersionBarFacade`），并将 feature 侧直接 `ImmersionBar.with(...)` 的调用逐步迁移；迁移完成后评估是否可将 `api(project(":repository:immersion_bar"))` 降级为 `implementation(...)` | 1) 新增入口可覆盖现有常见用法（透明/fitSystemWindows/字体颜色等）；2) 至少迁移 2 个调用点验证可行（例如 `BaseActivity` 与 `BaseSplashActivity`）；3) 不影响 TV/移动端交互与样式；4) 迁移过程保持渐进（允许双栈共存一段时间） | P2 | AI（Codex） | Done | 2026-02-06：统一入口 `StatusBarStyle` 已覆盖基础场景；feature 层无 `ImmersionBar` 直接 import，`:core_ui_component` 已改为 `implementation(project(":repository:immersion_bar"))`。 |
...
| G-T0062 | 统一本地代理能力：抽取通用 Proxy 服务（headers/range/tls 策略一致），减少 `HttpPlayServer`/`VlcProxyServer` 逻辑重复 | G-F0055 | `core_storage_component/.../HttpPlayServer.kt`；`player_component/.../VlcProxyServer.kt`；`core_network_component`（OkHttp 安全策略） | 1) mpv/VLC 均可通过统一入口获得可播放 URL；2) Range/headers 行为在主要站点/协议下稳定；3) 默认 TLS 策略安全且可配置；4) 全仓编译通过 | P2 | AI（Codex） | Done | 2026-02-06：以 `LocalProxy` 对外承接代理能力并统一播放器侧“是否本地代理 + seek enable”判定；同时统一存储侧 `onRangeUnsupported` 与 TLS 策略类型映射，完成 `HttpPlayServer` 直连收敛。 |
| G-T0063 | 若继续使用 PanelSwitchHelper：补齐生命周期释放/资源回收，避免潜在泄漏 | G-F0048 | 调整 `SendDanmuDialog` 持有 helper 实例，并在 `dismiss()`/`onStop()` 中按上游 API 做释放；必要时封装为 `PanelSwitchController`（内部持有 helper）以统一管理 | 1) Dialog 关闭后无残留监听（LeakCanary/手动验证）；2) Back 键与沉浸式状态栏恢复行为不回退；3) 不影响现有 UI | P2 | AI（Codex） | Done | 2026-02-06：`REPO_PANEL_SWITCH-T002` 已移除发送弹幕整链路，仓内无 PanelSwitchHelper 调用点，生命周期释放风险不再成立。 |
...
| G-T0067 | 增加可复现的回归用例：覆盖多格式/多文件/大文件/异常压缩包的解压行为 | G-F0023,G-F0067 | 在 `core_storage_component/src/androidTest/...` 增加 instrumentation 用例（更适配 native so）；准备最小化样例压缩包（可放 `core_storage_component/src/androidTest/assets/` 或 `document/code_quality_audit/fixtures/` 并在 README 说明来源/生成方式） | 1) 覆盖：zip/7z（至少 2 种），以及“多次 write”场景；2) 验证：输出文件 hash/大小正确；3) 验证：异常压缩包/取消时资源回收 | P3 | AI（Codex） | Done | 2026-02-06：新增 `SevenZipUtilsInstrumentedTest`（zip/7z hash+size 校验、异常包返回 null、取消后资源可回收），并补充测试资产与 `document/code_quality_audit/fixtures/README.md` 生成说明。 |
| G-T0068 | 收敛 media3/字幕遥测相关包结构，提升可发现性与复用一致性 | G-F0068 | 以 `com.xyoye.data_component.media3` 为根：将 `data/media3`、`entity/media3`、`media3/mapper`、字幕遥测相关 bean 逐步归并为明确子包（例如 `media3.dto`/`media3.entity`/`media3.telemetry`） | 1) 新包结构有明确规则并写入 `document/code_quality_audit/config/audit_dimensions.md` 或模块报告备注；2) 迁移按批次进行，单批次可编译通过；3) 迁移后全仓检索路径清晰（同类类型不再散落） | P3 | AI（Codex） | Done | 2026-02-06：完成 `media3.dto`、`media3.entity`、`media3.telemetry.subtitle` 分包迁移，并批量更新调用方 import。 |
